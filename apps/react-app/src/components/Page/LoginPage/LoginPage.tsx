...
const LoginPage = () => {
  ...
  const handleSaveLogin = async (data: NewLogin) => {
    ...
    await login({
      ...
      onSuccess: (response) => {
        ...
        // ListoACT 11 - After the login is successful, save the accessToken in local storage and use the following to direct the user to the home page:
        const { protocol, host } = window.location;
        const signInUrl = `${protocol}//${host}/categories`;
